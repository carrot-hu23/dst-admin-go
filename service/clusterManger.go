...
func (c *ClusterManager) QueryCluster(ctx *gin.Context) {
	...
	db = db.Where("activate = ? and (parent_cluster_name = '' or parent_cluster_name IS NULL)", true)
	db2 = db2.Where("activate = ? and (parent_cluster_name = '' or parent_cluster_name IS NULL)", true)
