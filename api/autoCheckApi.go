...
func (m *AutoCheckApi) GetAutoCheckList2(ctx *gin.Context) {
	...
	if checkType == "" {
		...
		db.Where("check_type = ? and cluster_name = ?", checkType, cluster.ClusterName).Find(&dbAutoChecks)
	} else {
		db.Where("check_type = ? and uuid in ? and cluster_name = ?", checkType, uuidSet, cluster.ClusterName).Find(&dbAutoChecks)
	}
